...
import marinermod.cards.attacks.RisingSurge;
import marinermod.cards.powercards.BermudaTriangle;
import marinermod.cards.attacks.FloatingGarbage;
import marinermod.character.MarinerCharacter;
import marinermod.monsters.Barnacle;
import marinermod.potions.BasePotion;
import marinermod.relics.BaseRelic;
import marinermod.util.*;
...
@SpireInitializer
public class MarinerMod implements
    ...
    @SpireEnum
    public static AbstractCard.CardTags REEF;
    ...
    public static void initialize() {
        new MarinerMod();
        MarinerCharacter.Meta.registerColor();
    }
    ...
    private static String checkResourcesPath() {
        String name = MarinerMod.class.getName(); //getPackage can be iffy with patching, so class name is used instead.
        ...
        if (!resources.exists()) {
            throw new RuntimeException("\n\tFailed to find resources folder; expected it to be at  \"resources/" + name + "\"." +
                    ...
                    "\t\"private static final String resourcesFolder = checkResourcesPath();\"\n" +
                    "\tat the top of the " + MarinerMod.class.getSimpleName() + " java file.");
        }
    ...
    private static void loadModInfo() {
        Optional<ModInfo> infos = Arrays.stream(Loader.MODINFOS).filter((modInfo)->{
            ...
            return initializers.contains(MarinerMod.class.getName());
        }).findFirst();
        if (infos.isPresent()) {
            info = infos.get();
            modID = info.ID;
    ...
    public void receiveEditCharacters() {
        MarinerCharacter.Meta.registerCharacter();
    }
    ...
    public void receiveEditCards() {
        ...
        // loads Shop and Exotic cards for the character
        if (Loader.isModLoaded("borealiscards")) {
            BaseMod.addCard(new BermudaTriangle());
            BaseMod.addCard(new FloatingGarbage());
        }
    ...
    public void receiveOnBattleStart(AbstractRoom abstractRoom) {
        Barnacle.barnacleArrayList.clear();
    }

    @Override
    public void receiveOnPlayerTurnStart() {
        for (AbstractCard c : AbstractDungeon.player.drawPile.group) {
            if (c.cardID == RisingSurge.ID) {
                ((RisingSurge)c).submergedThisTurn = false;
            }
        }
